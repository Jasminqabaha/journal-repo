...
def ensure_fifo(path: str):
    if os.path.exists(path):
        ...
        if not pystat.S_ISFIFO(st.st_mode):
            os.remove(path); os.mkfifo(path, 0o666)
    else:
...
def write_header_if_new(md_path: Path, with_weather: bool, lat: float, lon: float):
    if md_path.exists() and md_path.stat().st_size > 0: return
...
def fork_and_push(repo_dir: Path, md_path: Path):
    ...
    if not script.exists():
        logging.error(f"push.sh not found at {script}"); return
    pid = os.fork()
    if pid == 0:
        try:
...
def start_daemon(args):
    ...
    if not repo_dir.exists():
        print(f"repo path not found: {repo_dir}", file=sys.stderr); sys.exit(1)
    ...
    p.start(); log.info(f"writer pid={p.pid}")
    ensure_fifo(FIFO_PATH)
    log.info(f"listening on FIFO {FIFO_PATH} (echo 'hello' > {FIFO_PATH})")
    while True:
        try:
            with open(FIFO_PATH, "r", buffering=1, encoding="utf-8") as fifo:
                for line in fifo: q.put(line.strip())
        except KeyboardInterrupt:
            log.info("shutting down by KeyboardInterrupt"); break
        except Exception as e:
            log.exception(f"fifo read error: {e}"); time.sleep(1)
    p.join(timeout=2)
...
def build_parser():
    p = argparse.ArgumentParser(prog="daily-journal-sync", description="Tiny queue-based journaling with socket weather + GitHub push")
